...
bars = ax.barh(eu_export_2021['material_label'], eu_export_2021['tonne'], color = cmap(0), edgecolor = 'black', capsize=7)

tick_positions = range(len(eu_export_2021['tonne']))
ax.set_yticks(tick_positions)
font_props = font_manager.FontProperties(family='sans-serif', size=14)
ax.set_yticklabels(eu_export_2021['material_label'], fontproperties=font_props)

fig.suptitle('Total export of the European Union in 2021 by materials, Millions tonnes', fontsize=16, fontweight="bold", fontfamily='sans-serif')
ax.invert_yaxis()

# Add values near the bars
for bar, value in zip(bars, eu_export_2021['tonne']):
    ax.text(value + 100000, bar.get_y() + bar.get_height()/2,
            f'{value / 1e6:.3f} M', va='center', color='black', fontproperties=font_props)
